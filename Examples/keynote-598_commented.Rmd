---
title: "Supplement for Interim Analysis and Multiple Testing Strategy in KEYNOTE-598 Study"
date: "`r Sys.Date()`"
output:
...
##### 0. Global parameters
# Number of hypotheses included in the testing strategy with FWER control. In
# this example, we have 3 hypotheses: OS, PFS, and ORR
numHyp        <- 3
# Allowed one-sided FWER. In this case, 2.5% one-sided
alphaTotal    <- 0.025
### These next three options tailor the output, but typically don't need to be
### changed
...
plotInPercent <- FALSE
###
# Time limit in plot. This may require some trial-and-error if you're unsure on
# the time frame for the study's final analysis
Tmax_atPlot   <- 60
# Study specific parameter for multiple testing procedure. C.f., the edges of
# 0.999 in the graphical test for KEYNOTE-598
mtParam       <- 0.999

##### 1. Enrollment
# Overall study enrollment rate object. Duration of enrollment is 20 mo, and
# then the flat rate is based on realising a sample size of 568 patients
...
hypotheses    <- tibble::tibble(
  # Hypothesis IDs; always just H1, H2, etc. No need to modify this
  id            = paste0("H", 1:numHyp),
  # Hypothesis 'tags', used in graph and output tables; should just be the names
  # of the hypotheses. In this case, OS, PFS, ORR
  tag           = c("OS", "PFS", "ORR"),
  # The fields 'regimen', 'ep', and 'suffix' can be pasted together internally
  # into a description field for the output table defining hypotheses. In this
  # example, the regimen is Pembro, but no further distinguishment is need about
  # endpoints and suffices
  regimen       = rep("Pembro", numHyp),
  ep            = c("OS", "PFS", "ORR"),
  suffix        = rep("all", numHyp), # E.g., for subgroups
  # Type of hypothesis (primary or secondary). This just improves the clarity of
  # the output; in this case we have OS and PFS as dual primary endpoints
  type          = c("primary", "primary", "secondary"),
  # initial weights in graphical multiple testing procedure. Here, the weights
  # reflect 1.9% alpha going to OS and 0.6% alpha going to PFS initially
  w             = c(0.019, 0.006, 0)/0.025,
  # Spending functions; use NULL if no group sequential test for Hi. OS and PFS
  # use Lan-DeMets O'Brien-Fleming spending, while ORR does not do
  ...
  # 'enrollment' and 'iaSpec' would define N. In this example, the OS events at
  # the final analysis is set as ~361, with then the final information levels
  # for PFS and ORR computed based on the interim timings
  hypN          = c(361, NA, NA),
  # In some cases, we may would need to define 'infoFr' and 'iaTime' explicitly
  # infoFr        = list(),
  ...
  # Allocation ratio; trt/control. 1:1 in this example
  allocRatio    = 1,
  # Prevalence of the hypotheses. No subgroups in this example, so all are set
  # to 1
  prevalence    = c(1, 1, 1),
  # Compute enrollment to each hypothesis using its prevalence and the
  # previously set enrollment information. This section does not need to be
  # edited
...
graphProc     <- function(s, hypNames = NULL) {
  # s - split parameter
  # We set the matrix up so that PFS and OS recycle 's' between them and '1-s'
  # to ORR. ORR recycles all its alpha to PFS
  m             <- matrix(0, numHyp, numHyp)
  m[1, 2]       <- s     # OS to PFS
  m[1, 3]       <- 1 - s # OS to ORR
  m[2, 1]       <- s     # PFS to OS
  m[2, 3]       <- 1 - s # PFS to ORR
  m[3, 2]       <- 1     # ORR to PFS
  if (!is.null(hypNames)) {
    colnames(m) <- rownames(m) <- hypNames
  }
  new("graphMCP", m = m, weights = hypotheses$w)
}
G             <- graphProc(mtParam,
                           hypNames = paste(hypotheses$id, hypotheses$tag, sep = ": "))

##### 5. Depict the graphical testing procedure (refer to ?gMCPLite::hGraph)
# See ?gMCPLite::gGraph for the definition of its many input arguments. Here,
# we let the function determine where the endpoints are placed in the diagram
graphFigure   <- gMCPLite::hGraph(
  nHypotheses     = numHyp,
  nameHypotheses  = paste(hypotheses$id, hypotheses$tag, sep = ": "),
...
checkInput(hypotheses, G)    # check the inputs TODO
main_objects <- exec_calc(hypotheses)
D            <- main_objects$D
